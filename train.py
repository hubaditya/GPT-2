...
class DataLoaderLite:
    ...
    def __init__(self, B, T, train=True):
        self.B = B
        self.T = T
        ...
        print(f"1 epoch for {split} has {len(self.tokens) // (B * T)} batches")
...
train_dataloader = DataLoaderLite(B, T)
val_dataloader = DataLoaderLite(B, T, train=False)

# tf32 matrix multiplication to increase speed
torch.set_float32_matmul_precision("high")
optimizer = model.configure_optimizer(weight_decay=0.1, betas=(0.9, 0.95), lr=6e-4, eps=1e-8)
...
for step in range(max_steps):
    ...
    optimizer.zero_grad()
    loss_accum = 0
    # gradient accumulation to match batch size and then update parameters
    ...
    t1 = time.time()
    dt = (t1 - t0) * 1000
    tok_per_sec = int(train_dataloader.B * train_dataloader.T * grad_accum_steps / (t1 - t0))
    # rough initial loss = -ln(1/50257). This can be used as a sanity check that the model architecture is okay 
